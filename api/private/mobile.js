...
function *incrementStat (stats) {
  ...
  for (var i in stats) {
    media = yield mediaService.findByFakeUrl(i, this.user);
    if (media) {
...
function *getComments(id) {
  var media = yield mediaService.findByFakeUrl(id, this.user),
    parentList = [],
    parentId,
    childList = [],
    sortedList = [];
...
function *postComments() {
  ...
  var media = yield mediaService.findByFakeUrl(data.id, this.user);
  if (!media) {
    this.status = 403;
    return;
  }
